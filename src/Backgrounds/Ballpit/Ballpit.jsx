...
class x {
  ...
  #d() { this.scene = new i(); }
  #p() {
    this.canvas = this.#e.canvas || (this.#e.id && document.getElementById(this.#e.id));
    if (!this.canvas) { console.error('Three: Missing canvas or id parameter'); }
  ...
  clear() {
    this.scene.traverse(e => {
      if (e.isMesh && typeof e.material === 'object' && e.material !== null) {
        Object.keys(e.material).forEach(t => { const i = e.material[t]; if (i && typeof i.dispose === 'function') i.dispose(); });
...
function isEventOn(elem, ev) {
  if (!ev) return true;
  const tgt = ev.target; if (!tgt) return false;
  if (typeof ev.composedPath === 'function') return ev.composedPath().includes(elem);
...
function S(e) {
  ...
  // 터치 기기면 완전 무시 (리스너 미등록)
  if (IS_TOUCH_DEVICE) { return { ...t, dispose() {} }; }
...
function M(e) {
  ...
  A.x = e.clientX; A.y = e.clientY; processInteraction(e);
}
function processInteraction(ev) {
  for (const [elem, t] of b) {
    if (!isEventOn(elem, ev)) { if (t.hover && !t.touching) { t.hover = false; t.onLeave(t); } continue; }
...
function C(e) {
  ...
  for (const [elem, t] of b) { if (!isEventOn(elem, e)) continue; const i = elem.getBoundingClientRect(); P(t, i); if (D(i)) t.onClick(t); }
}
function L() { for (const t of b.values()) { if (t.hover) { t.hover = false; t.onLeave(t); } } }
function TouchStart(e) { if (IS_TOUCH_DEVICE) return; if (e.touches.length > 0) {
  A.x = e.touches[0].clientX; A.y = e.touches[0].clientY;
  for (const [elem, t] of b) { if (!isEventOn(elem, e)) continue; const rect = elem.getBoundingClientRect();
    if (D(rect)) { t.touching = true; P(t, rect); if (!t.hover) { t.hover = true; t.onEnter(t); } t.onMove(t); } } } }
function TouchMove(e) { if (IS_TOUCH_DEVICE) return; if (e.touches.length > 0) {
  A.x = e.touches[0].clientX; A.y = e.touches[0].clientY;
  for (const [elem, t] of b) { if (!isEventOn(elem, e)) continue; const rect = elem.getBoundingClientRect(); P(t, rect);
    if (D(rect)) { if (!t.hover) { t.hover = true; t.touching = true; t.onEnter(t); } t.onMove(t); }
    else if (t.hover && t.touching) { t.hover = false; t.touching = false; t.onLeave(t); } } } }
function TouchEnd() { if (IS_TOUCH_DEVICE) return; for (const [, t] of b) { if (t.touching) { t.touching = false; if (t.hover) { t.hover = false; t.onLeave(t); } } } }
...
class W {
  ...
  #R() { const { config: e, positionData: t } = this;
...
class Z extends d {
  ...
  #S() { this.ambientLight = new f(this.config.ambientColor, this.config.ambientIntensity); this.add(this.ambientLight);
         this.light = new u(this.config.colors[0], this.config.lightIntensity); this.add(this.light); }
  setColors(e) {
    if (Array.isArray(e) && e.length > 1) {
      const t = (function (e) {
        let t, i; function setColors(e) { t = e; i = []; e.forEach(col => i.push(new l(col))); }
        setColors(e);
        return {
          setColors,
